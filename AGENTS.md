...
- The documentation finalization task is done. The two Markdown files in `docs/presentation/` stay the final submissions and the user chose to keep them only there; do not recreate Desktop copies. Keep presentation facts aligned with the canonical product documents, without requiring identical full text, and use the final submission filenames without intermediate version suffixes. `case-brief.md` stays verbatim.
