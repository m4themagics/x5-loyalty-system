...
- The current finalization task is documentation only: do not change runtime code, JSON schemas, fixtures, dependencies, commits, `case-brief.md`, PPTX or any presentation files. Preserve legacy presentation artifacts and label them historical in the index. Future capabilities remain unactivated in `CHECKLIST.md` until a separate implementation task.
