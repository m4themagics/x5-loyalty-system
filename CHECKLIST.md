...
Статус `absent` у согласованных будущих возможностей в реестре ниже обозначает отсутствие реализации и её активации в текущей версии. Их спецификация одобрена; код будет отдельной задачей. Текущая работа охватывает всю актуальную продуктовую документацию, в том числе полные Markdown-материалы в `docs/presentation/`. PPTX и дословный `case-brief.md` сохраняются. Входы, выходы, зависимости и приёмка следующей реализации определены в [плане проекта](docs/project/plan.md).
