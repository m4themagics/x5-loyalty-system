...
| What is the first user journey that must work end to end? | Целевой путь: история покупок → выполнимое задание → чек → обещанный предмет → коллекция → скидка. Локально связаны персональное задание до отдельного окна награды и игра «случайная коробка → коллекция → четыре предмета → скидка со штрихкодом». Инвентарь общий; коробка за три дня входа и купон до 100 ₽ реализованы локально. |
...
| Коробки за входы | available | Три разных дня по Москве, четыре получения за скользящие 28 дней, резерв 25 ₽ до показа прогресса; пропуски не сбрасывают дни. |
...
| Коллекция и создание скидки | included | Четыре слота, выбор нажатием/перетаскиванием, семь рецептов, расход копий, одна активная скидка до 100 ₽ (старые купоны сохраняют 10 ₽) и локальный EAN-13. Реального погашения нет. |
