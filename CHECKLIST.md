...
| Local PoC decision API and LLM card | available | The webapp calls `python3 recsys/engine/cli.py` over a Vite middleware at `/api/demo/decision` and `/api/demo/event`, validated by the shared Zod contract in `packages/contracts/src/demo-poc.ts`. The YandexGPT Lite adapter is written but unverified: with no credentials every card falls back to a validated deterministic template. Dev-server only. |
...
| Coupon lifecycle and maximum reward reservations | available | The shared PoC coupon fund is enforced end to end: RUB 10 cap, RUB 2.50 per promised or unspent instance, separate physical fund, full maximum liability reserved before a promise, and an active coupon blocking a new one until demo redemption. Browser-local; not a ledger. |
...
| Referral qualification and reward calculation | available | Demo referral conditions are calculated: no prior confirmed purchase, first qualifying purchase within seven days of the invite, at most one inviter reward per window, self-invites excluded. Calculation and display only; no invitation flow, event feed or issuance. |
