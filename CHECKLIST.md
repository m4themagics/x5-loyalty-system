...
| Coupon lifecycle and maximum reward reservations | absent | Approved future specification: separate physical and digital liabilities, a defined ruble cap per coupon and conservative full-cap reservation for each digital instance. Atomic crafting turns four instance reserves into one coupon reserve and releases the excess; a new coupon is blocked until the active one is redeemed or expired. Existing browser-local crafting replaces one displayed discount; no protected ledger or POS fulfillment. |
| Avatar progress and voluntary savings ranking | absent | Approved future specification: number of distinct completed recipes, 0–7, without progress for repeating the same recipe; optional pseudonymous ranking by confirmed redeemed savings over 28 days, with equal savings sharing a rank. The static character is present, but this progress and ranking are not connected. |
