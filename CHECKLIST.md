...
| What is the first user journey that must work end to end? | Целевой путь: история покупок → выполнимое задание → чек → обещанный предмет → коллекция → скидка. Сейчас локально реализованы две части: персональное задание до отдельного окна награды и независимая песочница «случайная коробка → коллекция → четыре предмета → скидка со штрихкодом». Их инвентари ещё не объединены. |
...
| Игровой профиль и случайная коробка | included | Маскот, уровень, выгода, безлимитная коробка, 24 предмета, шансы 70/25/5 и сохранение в `localStorage`. |
...
| Avatar progress and savings ranking | available | Уровень и рейтинг вычисляются по локальному синтетическому состоянию; внешность маскота не меняется. |
