...
Приоритет к Demo Day: один связный персональный сценарий с LLM-карточкой → независимая оценка 40 профилей (28/40), симуляция 1 000 пользователей и простой антифрод → расчёт прогресса/рейтинга/реферала → защита. Обмен, повторные товарные цели и полный аукцион сохраняются в плане развития и не блокируют проверки. Условие синтетического задания: одна оплаченная единица из указанной категории за семь дней; бесплатные строки и повторы не засчитываются, возвраты проверяются отдельно. Код пока не активируется.
...
Статус `absent` у согласованных будущих возможностей в реестре ниже обозначает отсутствие реализации и её активации в текущей версии. Их спецификация одобрена; код будет отдельной задачей. Текущая работа охватывает всю актуальную продуктовую документацию, в том числе компактные финальные материалы в `docs/presentation/` по требованиям сдачи: аннотация на 6–7 абзацев и обязательные разделы проекта; три продуктовых материала с вкладом AI Product. Дословный `case-brief.md` сохраняется. Входы, выходы, зависимости и приёмка следующей реализации определены в [плане проекта](docs/project/plan.md).
...
| Recipe goal selection and promised item issuance | absent | Approved future specification: choose and persist a useful recipe goal, score missing existing-catalog items and issue the displayed item once after qualification. The current nearest-recipe hint is local combination matching, not personalized goal selection. Four copies remain necessary for crafting; a first challenge from an empty inventory grants only the first item. |
...
| Coupon lifecycle and maximum reward reservations | absent | Approved future specification: a common PoC coupon fund with a uniform RUB 10 cap, RUB 2.50 per unspent or promised-but-unissued digital instance, plus issued unredeemed coupon maxima. Atomic crafting turns four instance reserves into one RUB 10 coupon reserve without releasing an excess; physical rewards and advertiser budgets remain separate; a new coupon is blocked until the active one is redeemed or expired. Existing browser-local crafting replaces one displayed discount; no protected ledger or POS fulfillment. |
| Avatar progress and voluntary savings ranking | absent | Approved future specification: number of distinct completed recipes, 0–7, without progress for repeating the same recipe; optional pseudonymous ranking by confirmed redeemed savings over 28 days, with equal savings sharing a rank. Savings can depend on purchase volume; rank does not increase reward and spend does not directly raise avatar level. The static character is present, but this progress and ranking are not connected. |
| Referral qualification and reward calculation | absent | Approved future specification: one promised common digital item for the inviter after the invited participant's first verified qualifying purchase; no payment for signup alone or double assignment of one event. New-participant criteria, qualification window and inviter cap must be configured before a campaign starts. Exchange is separate; no referral event feed, calculation or invitation screen is connected. |
| Habit, relevance and economics evaluation | absent | Evaluation protocol is documented. The planned independent 40-profile relevance assessment (at least 28 matches, separate refusal cases), 1,000-user simulation, fraud precision/recall evaluation and real 28-day purchase/margin measurements have not been produced by the current scripts. |
