...
| What is the first user journey that must work end to end? | Локально реализовано: история покупок и инвентарь → одно выполнимое задание → тестовый чек → обещанный предмет и демонстрационное право на товар → существующее создание скидки. Исходная коробка → предмет → четыре копии → скидка со штрихкодом сохраняется отдельной песочницей |
...
| Local demo profiles and progress | included | The mobile web demo includes an interactive profile, an expandable inventory with eight initial slots, static weekly tasks, and an unlimited reward chest. Every shake-opened chest randomly selects one of 24 items using documented 70%/25%/5% rarity weights, adds it to the persisted inventory, stacks duplicates, and is immediately available again. Tapping an inventory item opens its rarity and a playful usage clue without revealing the exact discount category. Four inventory items can be placed into discount slots by drag-and-drop or tap, consumed to craft a discount from a 5–15% base with a recipe bonus and an 18% formula cap, and matched against seven themed recipes for a +2/+3 percentage-point synergy bonus. Items are not worn by the mascot. One active discount is persisted, shown beside the character, and opens a structurally valid EAN-13 barcode. Inventory, randomization, crafting, and discount state are browser-local only: there is no backend profile sync, real purchase feed, task issuance, cooldown enforcement, anti-tamper randomization, discount expiry/redemption, POS registration, or production reward fulfillment. |
| Allocator scenario contract and validators | available | The legacy allocator JSON schema, campaign catalog, fixtures and validators remain. The live local path uses Zod contract v2 for profile, decision, event and Ads state; game screens consume this contract through Vite/Python rather than the legacy fixtures. |
| Local PoC decision API and LLM card | available | The webapp calls `python3 recsys/engine/cli.py` over a Vite middleware at `/api/demo/decision` and `/api/demo/event`, validated by contract v2 in `packages/contracts/src/demo-poc.ts`. Local Qwen3 1.7B runs through Ollama without a cloud key; YandexGPT remains optional. Invalid or unavailable output falls back to a validated template. Dev-server only. |
| Local X5 decision and evaluation view | available | The «Для X5» screen shows decision and `offer`/`no_action` reasons, Ads winner/score/pacing/frequency/budget, reserve and one-time billing, plus policy and learned-RecSys evaluations. It is not a production advertiser console. |
| Personal purchase challenge / RecSys | available | Runtime `recsys/engine` fully scores the small catalog with rules and serves different challenges for the demo profiles. Exploration is off. A separate standard-library learned contour implements treatment/control logistic regression, calibrated uplift and billable-event prediction on randomized synthetic logs; it is evaluated offline and not served. |
...
| Guaranteed first physical reward | available | The first cycle promises one gift SKU beside the digital item only when an eligible advertiser wins and covers its campaign obligation; stock and reward liability are checked before display. Repeat cycles stay digital. The demo grants an entitlement record only; no real issuance or redemption. |
| Digital item exchange | available | Local PoC only: a two-arrow entry beside the profile mascot opens a mobile exchange sheet with profile-style slots, a picker containing every currently available owned item, a demonstrational pairing QR/code, the other participant's slot and explicit confirmation. Anya/Boris synthetic profiles implement 1:1 same-rarity item exchange, including a single owned copy, 24h expiry, sender proposal and receiver confirmation, reservation, atomic local transfer, max 3 completed exchanges in rolling 7 days and 2 verified purchase days per user. A contract-valid built-in synthetic seed keeps this UI available when the local Python profile endpoint is absent on static hosting. Reload, acceptance, rejection and craft after exchange pass browser checks. The QR does not connect real devices and there is no server ledger. |
| Retail-media auction and billing | available | Local one-placement quality-adjusted first-price CPA auction with competing dairy/coffee campaigns; category/flight/budget/frequency/quality/increment filters; pacing only in allocation; bid+subsidy must cover the first physical SKU, is reserved at display, and is billed once after a fresh qualified `allow` event. The digital-item reserve stays in the separate coupon fund. Global demo ledger is browser-local; real advertisers, contracts, protected budgets and financial settlement are absent. |
...
| Habit, relevance and economics evaluation | available | Reproducible local runs exist: 40 profiles, eight refusal cases, four policies on five seeds of 1,000 users with liabilities and stress worlds, fraud metrics, and a separate learned-RecSys evaluation on randomized synthetic logs with deterministic held-out comparison. Live Qwen checks three card facts on 15 synthetic personas. Labels remain `agent_draft`; no human walkthroughs or real 28-day purchase/margin measurement. |
