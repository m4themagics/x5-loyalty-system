...
| Local demo profiles and progress | included | The mobile web demo includes an interactive profile, four discount slots, eight inventory slots, static weekly tasks, and a persisted reward-chest cycle (`counting → openable → claimed`). An expired chest remains openable across reloads; claiming it through the shake gesture starts the next 24-hour cycle. State is browser-local only: there is no backend profile sync, real purchase feed, task issuance, or production reward fulfillment.                                                                 |
