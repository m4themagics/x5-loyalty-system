...
Этот документ подробно описывает **действующую игру коллег**: коробка → цифровой предмет → инвентарь → четыре предмета → собственная скидка → демонстрационный штрихкод. Каталог и расчёт используются веб-прототипом. В тот же локальный интерфейс добавлены покупочное задание с гарантированной демонстрационной наградой и обмен доступных цифровых предметов одинаковой редкости; реальная выдача товара и защищённый серверный учёт остаются следующей интеграцией.
...
Локальный PoC сохраняет эти 24 предмета, семь рецептов и взаимодействие с четырьмя слотами. RecSys выбирает одно допустимое задание и полезный цифровой предмет из того же каталога; текущий интерфейс показывает его в отдельном окне и добавляет в общую коллекцию. Будущий сервер должен фиксировать обещанный ID **до показа задания** и переносить именно этот предмет в защищённый единый инвентарь. Случайные вероятности демо 70/25/5 не могут заменить обещание.
