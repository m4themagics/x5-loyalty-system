...
Этот документ подробно описывает **действующую игру коллег**: коробка → цифровой предмет → инвентарь → четыре предмета → собственная скидка → демонстрационный штрихкод. Каталог и расчёт используются веб-прототипом. Отдельный локальный PoC уже добавляет покупочное задание с гарантированной демонстрационной наградой и обмен цифровых дубликатов; реальная выдача товара и защищённый серверный учёт остаются следующей интеграцией.
...
Следующая интеграция сохраняет эти 24 предмета, семь рецептов и взаимодействие с четырьмя слотами. RecSys уже выбирает одно допустимое задание и полезный цифровой предмет из того же каталога, но текущий интерфейс показывает его в отдельном окне и хранит отдельно от основной коллекции. Целевой сервер фиксирует обещанный ID **до показа задания** и затем переносит именно этот предмет в единый инвентарь. Случайные вероятности демо 70/25/5 не могут заменить обещание.
