...
test('the live Ads ledger reserves an impression, bills one CPA, survives reload, and ignores replay', async ({ page }) => {
  ...
  await expect(panel.getByText('Ads campaign', { exact: true })).toBeVisible()
  await expect(panel.getByText('CPA charged', { exact: true })).toBeVisible()
  await expect(panel.getByText('Available campaign budget', { exact: true })).toBeVisible()
  await expect(panel.getByText('Campaign reserve / exposure status', { exact: true })).toBeVisible()
  await expect(panel.getByText('Impressions in 14 days', { exact: true })).toBeVisible()
  const cpaValue = panel.locator('dt').filter({ hasText: 'CPA charged' }).locator('xpath=following-sibling::dd')
  const billedRubles = `RUB ${(billing.amount_kopecks / 100).toLocaleString('en-US', { maximumFractionDigits: 2 })}`
  await expect(cpaValue).toHaveText(billedRubles)

  await page.getByRole('button', { name: 'Close the X5 panel' }).click()
  await page.getByRole('button', { name: 'Full demo reset', exact: true }).click()
  await page.getByRole('button', { name: 'For X5', exact: true }).click()
  await expect(cpaValue).toHaveText('RUB 0')
...
test('exhausted live campaign budgets produce no_action before a first physical promise is shown', async ({ page }) => {
  ...
  await expect(page.getByRole('article', { name: 'Challenge card' })).toHaveCount(0)
  await expect(page.getByText('No suitable challenge right now', { exact: false })).toBeVisible()
  await openStand(page)
  await expect(page.locator('.demo-diagnostics')).toContainText('ads_budget_insufficient')
  await page.getByRole('button', { name: 'For X5', exact: true }).click()
  const panel = page.getByRole('region', { name: 'For X5', exact: true })
  const campaignValue = panel.locator('dt').filter({ hasText: 'Ads campaign' }).locator('xpath=following-sibling::dd')
  await expect(campaignValue).toHaveText('No active challenge')
