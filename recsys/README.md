...
| [llm/](llm) | Адаптер YandexGPT Lite, проверка карточки и детерминированный шаблон |
...
Проверяемые следующие артефакты: независимая разметка 40 допустимых профилей с целью 28/40 и отдельная проверка отказов,
симуляция 1 000 пользователей с явными допущениями, сценарии положительной/отрицательной
...
Проверки не требуют новых зависимостей. Они не запускают запланированную симуляцию 1 000 пользователей
и не измеряют релевантность на выбранных 40 профилях: эти результаты ещё нужно получить.

Уточнение будущего контракта PoC: единый купонный фонд, максимум купона 10 ₽ и резерв 2,50 ₽ на непотраченный или обещанный экземпляр; созданные непогашенные купоны учитываются отдельно. Физические SKU и бюджеты брендов обеспечиваются отдельно. Четыре копии плюс товар себестоимостью 25 ₽ требуют 35 ₽; бюджет 35 000 ₽ даёт до 1 000 таких предложений до остальных расходов. Первый товар плюс одна копия требуют 27,50 ₽; стоимость всего будущего пути оценивается отдельно. Квалификация демо — одна оплаченная единица указанной категории за семь дней; бесплатные строки и повторный чек не засчитываются, возврат проверяется отдельно. Эти поля и расчёты в текущие фикстуры и валидаторы не добавлены.
