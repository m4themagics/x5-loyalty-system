...
Обученные модели, рекламный аукцион и серверный реестр прав по-прежнему не реализованы.
Локальный обмен предметами реализован в webapp и использует общий браузерный снимок состояния.
...
в локальном демонстрационном пути; локальный обмен подключён в webapp, а Ads и промышленный
антифрод остаются следующим шагом.
...
| [eval/](eval) | Независимая разметка, парное сравнение политик, симуляция аудитории, Qwen-проверка текста и оценка антифрода |

Путь целиком: история покупок → вычисленное задание → проверенная карточка → тестовый чек →
обещанные награды → обмен дубликатами → существующий крафт. Панель «Для X5» читает компактный
срез воспроизводимого отчёта политик. Учёт демонстрационный, в одной вкладке браузера;
...
Промышленный обмен и серверный учёт наград требуют отдельного контракта и реализации;
локальный обмен уже проверяет резервирование и однократную передачу в общем снимке браузера.
...
1 560,40 ₽, диапазон 1 398,90–1 805,50 ₽, 5/5 seed при охвате 16,12%. Разметка ждёт
