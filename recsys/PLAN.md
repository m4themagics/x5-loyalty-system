...
| Следующая награда | Цифровой предмет; физический SKU только при явно заданном milestone или полном финансировании кампании |
...
Победитель определяет единственный показанный челлендж. Органический вариант используется
при `no_fill`; второй параллельный слот или конкурирующее обещание не создаётся.
...
Для первой безопасной реализации предлагаем консервативное правило: каждый обещанный или
выданный цифровой экземпляр обеспечивается **полным максимумом одного будущего купона**.
Максимум общий для поддерживаемых рецептов и фиксируется в рублях до запуска. Это избыточный
резерв, зато без предположения, что четыре четверти обязательства всегда встретятся у одного
владельца. Значение максимума, разрешённый ассортимент, срок и правила сочетания промо
требуют данных X5; без них настоящая скидка не выдаётся.

При крафте четыре резерва атомарно заменяются одним полным резервом купона, лишнее освобождается.
Погашение переводит его в фактический расход; остаток освобождается. При законном истечении
именно купона освобождается его резерв. Истечение задания не освобождает уже выданные предметы
и не создаёт право молча отменить обещанный результат. Срок жизни предметов, если он понадобится,
должен быть отдельным заранее объявленным правилом; сейчас автоматического сгорания не планируем.

Обмен переносит резервы вместе с происхождением экземпляров, без начисления новых денег.
Общий максимум предотвращает недофинансирование при новом сочетании, но ожидаемая стоимость
меняется из-за вероятности погашения, поэтому экономический допуск обмена всё равно нужен.
Более точный совместный расчёт обязательств допускается позже после отдельного доказательства
верхней границы расходов. Делить резерв купона на четыре без такого доказательства нельзя.

Целевой сервер не стирает прежний неиспользованный купон новым крафтом: до погашения или
объявленного истечения имеющегося купона новый крафт недоступен. Текущее демо хранит одну
...
### 7.1. Границы будущих операций

Следующие сущности и названия — проект контракта для реализации; они **не добавлены**
в `action.schema.json`, fixtures, API или базу. Общий идентификатор каталожного предмета
не заменяет идентификатор конкретного принадлежащего пользователю экземпляра.

| Операция | Вход | Выход и инвариант |
| --- | --- | --- |
| Выбор | Снимок покупок, доступных/зарезервированных экземпляров, выбранного рецепта, кампаний, SKU и риска | `decision_id`, выбранный кандидат либо `no_action`, причины и оценки всех кандидатов; финансовых прав пока нет |
