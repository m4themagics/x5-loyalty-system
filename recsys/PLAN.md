...
Qwen/fallback-карточка, симулятор аудитории и политик, простой fraud scorer и браузерный обмен
дубликатами. Обученные модели, рекламный аукцион, промышленная выдача и серверный реестр
не реализованы. Ниже отделены работающий PoC и следующий производственный этап.
...
| Готово в PoC | Показ выбора и расходов для продакта / маркетолога | Панель «Для X5» показывает политику симуляции, финансирование, обязательства, риск и источник карточки |
...
Это не пользовательское исследование и не подтверждение качества будущей модели вероятностей.

На защите: показать игровой результат и обмен; объяснить работающий rules-based выбор задания;
раскрыть источник награды, резерв, риск и результат политики на панели «Для X5». Полный Ads-аукцион
и обучаемые модели описываются как следующий этап. Числа симуляции помечаются как сценарные,
а вклад ИИ команды подтверждается конкретными использованными инструментами и артефактами.
