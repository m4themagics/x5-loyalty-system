...
Реальные значения и условия согласуются с X5 перед пилотом; схема и runtime пока не меняются.
...
| До Demo Day | Rules-based baseline, LLM-карточка и независимая разметка | Минимум 28/40 на профилях с допустимым заданием; разметка до оценки, отказы отдельно; сейчас результата нет |
...
| До Demo Day | Краткая симуляция 1 000 пользователей | Фиксированный seed; положительный, нулевой и отрицательный эффект; таблица охвата, расходов, резервов и маржи; результат не называется эффектом X5 |
...
| До Demo Day | Расчёт прогресса аватара, рейтинга и реферала | Подписанные синтетические события; повтор не увеличивает прогресс и не выдаёт вторую награду |
| Следующий этап | Товарные цели, безопасный обмен и промышленный серверный учёт | Атомарность, идемпотентность, ограничения, сохранение обязательств |
