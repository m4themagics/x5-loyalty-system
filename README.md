...
[Прогон защиты на 4:30](docs/project/demo-runbook.md) · [короткие ответы](docs/project/demo-qa.md).
...
Основной персональный путь, квалификация чеков, резервы, сохранение прогресса и Qwen/fallback-карточка проверены локально. Следующий шаг — 5–7 пользовательских прохождений, подтверждение рубрики человеком, репетиция и резервная запись. Результаты и ограничения собраны в [статусе PoC](docs/project/poc-status.md).

Локальный обмен дубликатами уже позволяет завершить рецепт: Аня отдаёт лишний кувшин Борису и получает недостающую сковороду. Товарные цели повторных циклов и рекламный CPA-аукцион входят в развитие концепции. В сравнении на пяти seed финансируемый первый подарок дал в среднем **+1 560,40 ₽ на 1 000 синтетических пользователей после обеспечения новых непогашенных наград**, **5/5** положительных итогов при охвате **16,12%**. Эта политика пока работает в оценщике, а не в выборе задания интерфейса; допущения и альтернативы приведены в [отчёте](recsys/eval/README.md). Обученные модели, реальная выдача и серверный учёт ещё не подключены; эффект на покупателях предстоит проверить.
