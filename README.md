...
Награда персонального задания показывается отдельным полноэкранным окном и попадает в ту же «Коллекцию», что и предмет из коробки: инвентарь и четыре слота сборки в продукте одни. Каждый экземпляр — и выпавший из коробки, и выданный за задание — удерживает резерв 25 ₽ под будущий купон до 100 ₽. Резерв коробки создаётся перед показом прогресса и переходит предмету без повторного удержания. Бюджет демо не увеличен; старая коллекция сохраняется с пересчётом обеспечения, старые купоны — со своим максимумом.
...
7. Покажите процент, потолок 100 ₽, пример фактической экономии и демо-штрихкод. Старые купоны сохраняют потолок 10 ₽.
