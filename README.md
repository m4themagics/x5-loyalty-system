...
В текущем интерфейсе награда персонального задания показывается отдельным полноэкранным окном и хранится в состоянии персонального PoC. Она **не переносится в коллекцию случайной коробки и не участвует в её четырёх слотах**. Поэтому это два доказанных локальных сценария, а не один полностью связанный путь от AI-задания до штрихкода.
...
В «Демо-стенде» выберите «Аня · обмен», затем откройте кнопку «Обмен» рядом с маскотом. Выберите свой предмет, покажите QR, создайте предложение и переключитесь на «Борис · обмен» для принятия. QR и соединение участников демонстрационные; передача выполняется между локальными синтетическими профилями.
...
На текущем состоянии проходят **208 автоматизированных unit/contract-проверок**:
...
Также проходят три валидатора RecSys/LLM. В репозитории есть 18 Playwright-сценариев, но после последнего изменения структуры профиля часть старых сценариев всё ещё ожидает удалённый блок персонального инвентаря и кнопку демо-погашения. До их обновления полный браузерный пакет не считается зелёной приёмкой.
