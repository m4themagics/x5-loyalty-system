...
- The active task is the local proof of concept: purchase history -> computed challenge -> validated LLM card -> synthetic receipt -> promised rewards -> existing crafting, plus avatar, ranking and referral calculation. It runs on one laptop in one browser tab and is explicitly demonstrational. Never describe it as a protected server ledger, a real recommender in production or actual product fulfillment.
