...
- `recsys/schema/action.schema.json` is the machine-readable contract between the allocator and every client, and it is authoritative for field names, enums, and required fields. A decision carries a campaign, a creative, a mechanic family, a reinforcement, the economics of the placement (bid, pacing multiplier, expected spend), the calibrated probabilities, the propensity it was logged with, and its reason codes. The same shape is written out in section 8.3 of the project description and Material 6 of the product materials; all three change together.
- Every decision carries three independent levels, and they must never be collapsed into one field: `experiment_arm` (`holdout` or `serve`, assigned at random and fixed per user), `sponsored_result` (`filled` or `no_fill`), and `surface_result` (`sponsored`, `organic`, or `none`). `no_fill` together with `organic` is the normal case, not an error: the advertising was refused and the person still gets a route. A holdout gets no impression at all, only a ghost record.
- A ghost record says what the system would have shown. It says nothing about what the person would have done, so never compute a lift by subtracting a ghost prediction; lift comes from comparing randomised groups over everyone assigned to serve, including those the policy chose not to treat.
- Propensity is stored decomposed - `experiment`, `action_given_serve`, `creative_given_campaign` - and their product is the logging propensity. A single opaque number cannot be used to reweight anything later.
- A sponsored route always carries a visible sponsorship label. `recsys/validate_explanation.py` refuses a sponsored creative without one, and refuses copy that names a price, invents an SKU, restates a different deadline, promises a causal effect, or releases a hold.
- Never rank by media value alone. A candidate is refused when the estimated increment over the untreated baseline falls below the incrementality floor, even when the campaign is willing to pay and its media value is the highest on the table - selling an advertiser a visit that would have happened anyway is the failure this system exists to prevent.
- The bid is set by the advertiser and is never predicted by a model. Models predict the probability of a billable event; the bid multiplies it. Campaign budget is spent against `total - settled_spend - pending_liability`, because a delayed conversion means impressions are served long before they are charged.
- Client screens read the fixtures in `recsys/fixtures/` and the campaign catalog in `recsys/catalog/campaigns.json` instead of hard-coding profiles, routes, rewards, counters, or task lists in a component. Each fixture carries `profile`, `decision`, `creative_copy`, and `route_state`. If a screen needs something they do not carry, add the field to the schema and both documents, or add a fixture; do not invent a private data shape next to them.
- After changing the schema, a fixture, or any user-facing copy, run `python3 recsys/validate.py` and `python3 recsys/validate_explanation.py`.
- Do not build a parallel product structure inside this repository. The specification lives in `docs/project/`, the current plan and ownership in `docs/project/plan.md`, the contract, catalog and demo data in `recsys/`. A new directory for mock data, demo profiles, or product constants duplicates one of those and is not wanted.
