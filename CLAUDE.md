...
- `fill_type` is part of every decision: `sponsored` when a campaign pays, `organic` when the route runs without an advertiser, `no_fill` when no campaign clears the bar, `no_action` when nothing is shown at all. A screen must render all four; treating `no_fill` as an error state is wrong.
...
- Never rank by media value alone. A candidate is refused when the estimated increment over the no-treatment baseline falls below the incrementality floor, even when the campaign is willing to pay and its media value is the highest on the table - selling an advertiser a visit that would have happened anyway is the failure this system exists to prevent.
