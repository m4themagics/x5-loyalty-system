...
- Later physical rewards occur only at rare milestones or in fully funded campaigns. The repeatable result is digital items, duplicate exchange and crafted discounts. Keep both funding hypotheses: a brand funds the SKU, or X5 funds it when conservative expected incremental margin covers all associated costs.
...
- The current finalization task covers product documentation, the two Markdown files in `docs/presentation/`. The user chose to keep the final materials only there; do not recreate Desktop copies. Keep the presentation Markdown aligned with the canonical product documents; the `v4` filenames are retained compatibility names, not their content version. Do not change runtime code, JSON schemas, fixtures, dependencies, commits, `case-brief.md` or PPTX in this task. Label the unchanged PPTX historical in the index. Future capabilities remain unactivated in `CHECKLIST.md` until a separate implementation task.
