...
- The local integration is purchase history and inventory -> one feasible personal challenge -> Ads allocation -> a verified event -> one-time billing and the promised useful digital item -> existing discount crafting. After the first displayed, eligible, completed advertiser-funded challenge, the user also receives one full free physical SKU. The promise is guaranteed once shown and fulfilled under its stated conditions; a random drop must not replace it.
...
- Current gameplay has 24 items, seven recipes, a base discount plus recipe bonus with an 18% formula cap and a demonstration box earned over three distinct login days, capped at four claims per 28 days. A separate local PoC connects rules-based RecSys, a validated Qwen/fallback card, synthetic receipt qualification, one-time digital and physical-entitlement grants, coupon reservation/redemption, avatar level, savings ranking, referral calculation and same-rarity duplicate exchange. It also runs a local quality-adjusted first-price CPA Ads auction with browser-local budgets, impression reservations and one-time billing. An offline learned RecSys implements treatment/control logistic regression, calibrated uplift and a billable-event model on randomized synthetic logs; runtime still uses rules. Everything remains browser-local and demonstrational. Real fulfillment, POS, protected server ledgers, production bidder accounts and training on X5 data are not connected.
- In the target flow, show one next challenge. `no_action` applies before a new challenge is shown and preserves earned inventory and outstanding promises. The receipt total must not directly raise avatar level or linearly increase rewards. The ranking compares friends by collected sets and items only: money never enters a public comparison, because ranking people by spending pushes them to buy more. Confirmed redeemed savings stay a private number in the profile header, and rank never increases reward value. The collection title is written by the LLM over collected items; its validator rejects digits, money, discounts and promises and falls back to a deterministic template.
- First item-trading scope is 1:1 exchange of any owned, currently available digital items of the same rarity: 24-hour expiry, both parties confirm, items are reserved and transferred atomically, at most three completed exchanges per user per week, and at least two verified purchase days per participant. A single owned copy may be offered; once reserved it is unavailable to crafting or another trade. Physical goods, coupons, active discounts and unissued rewards cannot be transferred. Equal rarity does not guarantee equal cost: account for changed recipe completion and future discount redemption.
...
- `recsys/schema/action.schema.json` is authoritative only for legacy allocator scenario field names, enums and fixtures. `packages/contracts/src/demo-poc.ts` contract v2 is authoritative for the live local profile/decision/event/Ads wire path. Exchange state remains webapp-local and repeat physical goals remain future. Keep section 8.3 of the project description and Material 6 aligned with both boundaries. Current screens use game modules, not allocator fixtures; do not duplicate the game catalog.
...
- RecSys is a next-best-action recommender over a small fully scored candidate catalog. Runtime uses rules. The offline learned implementation uses separate `LogisticRegression` treatment/control models, `IsotonicRegression` calibration and a separate billable-event model on randomized synthetic logs; it is not runtime serving or evidence of X5 uplift. Two-tower retrieval is unnecessary at this scale. Missing purchase history in a category supports bounded exploration of related safe categories, not an assertion of interest.
- Ads is one placement with a local closed quality-adjusted first-price CPA auction. Apply category, flight, availability, frequency, risk, budget, quality and minimum expected-increment filters first. Rank eligible campaigns using expected brand payment, incremental X5 margin, pacing and uncovered reward cost. Reserve bid plus subsidy before display; the winner pays its submitted bid only once after a fresh verified qualifying event. Pacing changes allocation priority, never the billing price. The first physical reward requires an advertiser winner; later organic digital challenges are allowed. No useful challenge means no new task.
...
- PoC coupon promises share one funded pool and a uniform RUB 100 coupon cap: reserve RUB 25 for each unspent or promised-but-unissued item instance, plus the fixed maxima of issued unredeemed coupons. Never count a promise and its fulfilled instance twice. Crafting consumes four item reserves into one RUB 100 coupon reserve; local exchange preserves the pooled reserve. Physical SKU and advertiser CPA/subsidy budgets remain separate. Login boxes require three distinct Moscow calendar days, at most four claims in a rolling 28-day window. Reserve one item before showing progress; gaps preserve progress. Existing coupons retain their original cap. All rewards remain demonstrational.
...
- Demo acceptance covers the integrated personal challenge and LLM card, local Ads allocation/reservation/billing, independent relevance on 40 eligible profiles, paired policy evaluation on 1,000 synthetic users, offline learned-policy evaluation, simple fraud scoring, avatar/ranking/referral calculation, local duplicate exchange and the X5 evaluation panel. Repeat SKU goals and production Ads/backend remain later work.
...
- Keep product specifications in `docs/project/`, ownership in `docs/project/plan.md` and decision contracts/examples in `recsys/`. Synthetic simulations and offline models must not be presented as measured user behavior, X5 uplift or production quality; proposed user tests must not be presented as completed. Explain actual team use of AI as requested by the case Q&A; do not invent tool-use evidence.
- The documentation finalization task is done. The hackathon submission was handed over and its materials were removed from the repository: `docs/presentation/`, plus the defense artifacts `demo-qa.md`, `demo-runbook.md` and `technical-speech.md`. Do not recreate them, on the Desktop or in the repository, unless the user asks. The canonical product documents in `docs/project/` are now the only specification. `case-brief.md` stays verbatim.

### Local PoC Scope

- The local proof of concept covers purchase history -> computed challenge -> validated LLM card -> synthetic receipt -> promised rewards -> duplicate exchange -> existing crafting, plus avatar, ranking, referral calculation and a compact X5 evaluation panel. It runs on one laptop in one browser tab and is explicitly demonstrational. Never describe it as a protected server ledger, a real recommender in production or actual product fulfillment.
- `packages/contracts/src/demo-poc.ts` is the single wire contract for both sides. `recsys/contract/README.md` records the boundaries, the directory ownership and the reference payloads in `recsys/contract/examples/`. Change the contract only as one coordinated edit that updates the Zod schemas, the examples, the Python engine and the webapp together.
- Ownership is disjoint by directory: decision engine, economics and the LLM adapter in `recsys/engine/` and `recsys/llm/`; game integration in `webapp/`; independent evaluation in `recsys/eval/`. The contract files, `webapp/src/features/home/demo-game-snapshot.ts` and `webapp/scripts/build-demo-contract-examples.ts` are coordinator-owned.
- No new services, no FastAPI, no database, no new dependencies. The webapp reaches Python through a Vite `configureServer` middleware that spawns a fixed argv without shell interpolation; the engine is Python standard library only and speaks versioned JSON on stdin/stdout.
...
- Money is integer kopecks. One shared coupon fund caps a new coupon at 10000 and reserves 2500 for every unspent or promised-but-unissued instance; physical SKU and advertiser budgets stay separate. Reserve the full maximum liability before a promise is shown.
...
- Repeat physical goals, production model serving, real advertiser infrastructure and any public deployment stay outside this package. Local exchange, the bounded Ads auction and offline synthetic model training are included. The local demo uses Qwen3 1.7B through Ollama; YandexGPT remains optional. An unavailable or invalid model response falls back to a valid template and reports `source: "fallback"`.
